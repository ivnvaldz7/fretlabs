...
export function exportSvg(result: FretboardResult, options: ExportOptions): string {
  ...
  const fretSvgLines = fretLines.map((fl) => {
    ...
    return `  <line x1="${f(fl.x1)}" y1="${f(fl.y1)}" x2="${f(fl.x2)}" y2="${f(fl.y2)}" stroke="${stroke}" stroke-width="${strokeWidth}" stroke-linecap="round"/>`;
  }).join('\n');
  ...
  const numFrets = fretLines.length - 1; // fretLines includes fret 0 (nut)
